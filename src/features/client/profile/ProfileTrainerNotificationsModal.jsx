        ...
        {tasks.length > 0 ? (
          <>
            <p className={styles.summary} data-testid="profile-trainer-notifications-summary">
              {activeCount > 0
                ? isBasicPlan ? `Непрочитанных и активных: ${activeCount}` : `${activeCount} ${activeCount === 1 ? "активная задача" : "активных задач"}`
                : isBasicPlan ? "Всё прочитано и выполнено" : "Все задачи выполнены"}
            </p>
            <div className={styles.list} data-testid="profile-trainer-notifications-list">
              {tasks.map((task) => {
                ...
                const messageTitle = isBasicPlan
                  ? isReplyNotification ? "Ответ на сообщение" : "Сообщение"
                  : isReplyNotification ? "Ответ тренера" : "Сообщение от тренера";
                ...
                return (
                  <article
                    ...
                    aria-label={isMessageNotification
                      ? `${messageTitle}${messageContext ? `: ${messageContext}` : ""}. ${messageText || task.title}. ${messageRead ? "Прочитано" : "Новое"}`
                      : `${isBasicPlan ? "Напоминание" : "Задача тренера"}: ${task.title}. ${taskStatus.label}. ${taskDueText}`}
                  >
                    ...
                    <span className={styles.itemText}>
                      <strong className={styles.itemTitle}>{isMessageNotification ? messageTitle : task.title}</strong>
          ...
          <div className={styles.empty} data-testid="profile-trainer-notifications-empty">
            ...
            <p className={styles.emptyText}>{isBasicPlan ? "Сообщения и напоминания появятся здесь." : "Задачи и рекомендации тренера появятся здесь."}</p>
          </div>
        )}
      </section>
    </div>
